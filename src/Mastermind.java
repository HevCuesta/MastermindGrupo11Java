...
public class Mastermind {
	...
	public static void tutorial() {
		System.out.println("BIENVENIDO A MASTERMIND! \r"
				...
				+ CYAN + ' ' + RESET + BLANCO + ' ' + RESET + AZUL + ' ' + RESET + MORADO + ' ' + RESET + VERDE + ' '
				+ RESET + "\r\n"
	...
	public static char generarCombinacion(int i) {
		...
		case 0:
			combinacionSecreta[i] = 'C';
			break; // cyan
		case 1:
			combinacionSecreta[i] = 'R';
			break; // rojo
		case 2:
			combinacionSecreta[i] = 'V';
			break; // verde
		case 3:
			combinacionSecreta[i] = 'A';
			break; // amarillo
		case 4:
			combinacionSecreta[i] = 'Z';
			break; // azul
		case 5:
			combinacionSecreta[i] = 'M';
			break; // morado
		case 6:
			combinacionSecreta[i] = 'B';
			break; // blanco
		}
	...
	/**
	 * @return boolean para validar si la respuesta tiene los colores disponibles y
	 *         es de la longitud necesaria
		...
		try {
			...
			for (int i = 0; i < 5; i++) {
				...
				if (combinacion[i] != 'R' && combinacion[i] != 'C' && combinacion[i] != 'V' && combinacion[i] != 'A'
						&& combinacion[i] != 'Z' && combinacion[i] != 'M' && combinacion[i] != 'B') {
					System.out.println("Combinación no válida, inténtelo de nuevo. \r"
	 ...
	 * @param resultadoTablero Parte del tablero dedicada a la validacion de la
	 *                         combinacion
	 */
	...
	public static void nuevoJuego() {
		// ESTE SCANNER, NO SE PUEDE CERRAR DE NINGUNA MANERA, DA IGUAL COMO LO
		// INTENTEMOS, NO PODEMOS
		...
		while (WriteCSV.nuevaPartidaUsuario(nombreUsuario))
		...
		for (int i = 0; i < combinacionSecreta.length; i++)
			combinacionSecreta[i] = generarCombinacion(i);

		do {
			System.out.println(
					"Introduce la combinación de colores a adivinar (Para salir en cualquier momento, presione 0):");
			...
			if (combinacion[0] == '0') {
				...
				if (validarRespuesta(combinacion) == true && salir == false) {
					for (int i = 0; i < 5; i++)
						tablero[intentos][i] = combinacion[i];
				...
				WriteCSV.escribirArchivoGuardar(combinacionSecreta, intentos,
						"partidas/Usuarios/" + nombreUsuario + "/PartidaGuardada.csv");

			}

		} while (posibleAcierto(resultado) == false && salir == false);

		WriteCSV.guardarTablero(tablero, intentos, resultadoTablero,
				"partidas/Usuarios/" + nombreUsuario + "/TableroGuardado.csv", false);
	 ...
	 * @param combinacionSecretaGuardada Combinacion secreta guardada de la anterior
	 *                                   partida
	 * @param intentosGuardados          Numero de intentos guardados de la anterior
	 *                                   partida
	 */
	public static void partidaGuardada(String usuario) {
		...
		int intentosGuardados = ReadCSV
				.intentosPartidaGuardada("partidas/Usuarios/" + nombreUsuario + "/PartidaGuardada.csv");
		// En caso de que el archivo no exista, la funcion nos devuelve 9999 y eso nos
		// lleva de vuelta al menu
		if (intentosGuardados == 9999)
			return;

		char[] combinacionSecretaGuarda = ReadCSV
				.leerCombinacionGuardada("partidas/Usuarios/" + nombreUsuario + "/PartidaGuardada.csv");
		...
		ReadCSV.leerTablero("partidas/Usuarios/" + nombreUsuario + "/TableroGuardado.csv",
				ReadCSV.numLineasArchivo("partidas/Usuarios/" + nombreUsuario + "/TableroGuardado.csv"));

		char tablero[][] = new char[50][5];
		char combinacionSecreta[] = new char[5];
		for (int i = 0; i < 5; i++) {
			combinacionSecreta[i] = combinacionSecretaGuarda[i];
		...
		do {
			...
			if (combinacion[0] == '0') {
				...
				WriteCSV.escribirArchivoGuardar(combinacionSecreta, intentos + intentosGuardados,
						"partidas/Usuarios/" + nombreUsuario + "/PartidaGuardada.csv");
		...
		WriteCSV.guardarTablero(tablero, intentos, resultadoTablero,
				"partidas/Usuarios/" + nombreUsuario + "/TableroGuardado.csv", true);
	...
	public static void main(String[] args) {
		...
		while (restart) {
			...
			case 3:
				...
				System.out.println(
						"Presione 3 y enter otra vez si quiere ver todos los resultados, presione cualquier otra tecla para volver al menu.");
				if (in.next().equals("3")) {
					numLineas = ReadCSV.numLineasArchivo("partidas/Top5.csv");
					ReadCSV.leerArchivo("partidas/Top5.csv", numLineas);
					System.out.println(
							"Recuerda que el numero de la derecha son los intentos. Menos intentos = Mejor resultado");
					break;
